...
async function GET() {
    try {
        ...
        const { data: profile } = await supabase.from("profiles").select("is_premium, plan_tier, subscription_status").eq("id", user.id).single();
        const isPremium = profile?.is_premium || profile?.plan_tier === "premium" || profile?.subscription_status === "active";
        ...
        const { count } = await supabase.from("messages").select("*", {
            count: "exact",
            head: true
        }).eq("role", "user").eq("created_at", `gte.${startOfDay}`).eq("user_id", user.id);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            used: count ?? 0,
            limit: DAILY_LIMIT,
            isPremium: false
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7dbef02d._.js.map