...
export default function JournalPage() {
  ...
  async function runAI(mode: string) {
    setAiLoading(true);
    setAiResponse(null);
    if (mode === "weekly_roadmap") {
      trackClientEvent(ANALYTICS_EVENTS.BOARD_REVIEW_TRIGGERED, {
    ...
    if (!res.ok) {
      if (mode === "action_plan") {
        trackClientEvent(ANALYTICS_EVENTS.ACTION_PLAN_GENERATION_FAILED, {
          anonymous_id: getAnalyticsAnonymousId(),
          status: res.status,
        });
      }
      if (mode === "weekly_roadmap") {
        trackClientEvent(ANALYTICS_EVENTS.BOARD_REVIEW_FAILED, {
          anonymous_id: getAnalyticsAnonymousId(),
          status: res.status,
        });
      }
      setAiResponse(data.error || "No response.");
      setAiLoading(false);
      return;
    }
    ...
    if (mode === "weekly_recap" || mode === "weekly_roadmap") {
      trackClientEvent(ANALYTICS_EVENTS.WEEKLY_REVIEW_VIEWED, {
        anonymous_id: getAnalyticsAnonymousId(),
        review_type: mode,
      });
    }
    ...
    if (mode === "weekly_roadmap") {
      trackClientEvent(ANALYTICS_EVENTS.BOARD_REVIEW_COMPLETED, {
        anonymous_id: getAnalyticsAnonymousId(),
        source: "journal_page",
      });
    }
